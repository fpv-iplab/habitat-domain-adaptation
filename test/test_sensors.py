...
def test_rotation():
    ...
    config = get_config("configs/tasks/pointnav_orangedev.yaml")
    config.defrost()
    config.SIMULATOR.SCENE = (
        "data/scene_datasets/orangedev/env2.glb"
    )
    config.SIMULATOR.AGENT_0.SENSORS = ["RGB_SENSOR", "DEPTH_SENSOR"]
    config.freeze()
    ...
    #valid_start_position = [0, 0.08431, 0]
    valid_start_position = [2.5, 0.08431, -12.0]
    expected_pointgoal = [0.0, 0.2, -2.0]
    goal_position = np.add(valid_start_position, expected_pointgoal)
    ...
    state = env.sim.get_agent_state()
    actions = [
        "TURN_LEFT" for _ in range(N_STEPS)
    ]
    angle_diffs = []
    pos_diffs = []
    for step_id, action in enumerate(actions):
        if DEMO_MODE:
            x, y, z = quaternion.as_euler_angles(state.rotation)
            y = np.rad2deg(y)
            if abs(z)>0:
                y = 360.0 - y
            print("y angle:",y,", x,z:",x,z,", quat.:",state.rotation)
            #pil_image = Image.fromarray(obs["rgb"])
            #pil_image.save("test_images/angle_"+str(y)+"_quat_"+str(state.rotation)+".jpg")
