...
class PointGoalSensor(Sensor):
    ...
    def __init__(
        ...
        self._gps_noise_amount = getattr(config, "GPS_NOISE_AMOUNT", 0.0)
        self._rot_noise_amount = getattr(config, "ROT_NOISE_AMOUNT", 0.0)
    ...
    def _compute_pointgoal(
        ...
        if self._gps_noise_amount > 0.0:
            source_position = self._add_loc_noise(source_position)
        if self._rot_noise_amount > 0.0:
            x, y, z = quaternion.as_euler_angles(source_rotation)
            y = np.rad2deg(y)
            y = self._add_rot_noise(y)
            source_rotation = quaternion.from_euler_angles(x, np.radians(y), z)
    ...
    def _add_loc_noise(self, coordinates):
        # gps noise
        x_noise = np.inf
        z_noise = np.inf
        while np.abs(x_noise) >= self._gps_noise_amount: # truncate noise at 1 std
            x_noise = np.random.normal(0.0, self._gps_noise_amount, 1)[0]
        while np.abs(z_noise) >= self._gps_noise_amount:
            z_noise = np.random.normal(0.0, self._gps_noise_amount, 1)[0]
        coordinates[0] += x_noise
        coordinates[2] += z_noise

        if self._dimensionality == 3:
            y_noise = np.inf
            while np.abs(y_noise) >= self._gps_noise_amount:
                y_noise = np.random.normal(0.0, self._gps_noise_amount, 1)[0]
            coordinates[1] += y_noise
        return coordinates

    def _add_rot_noise(self, y_angle): # in degrees
        y_noise = np.inf
        while np.abs(y_noise) >= self._rot_noise_amount:
            y_noise = np.random.normal(0.0, self._rot_noise_amount, 1)[0]
        y_angle += y_noise
        if y_angle > 360.0:
            y_angle -= 360.0
        elif y_angle < 0:
...
class GlobalGPSSensor(Sensor):
    r"""The agents current location in the coordinate frame defined by the world
    ...
    def get_observation(
        ...
        if self._dimensionality == 2:
            return np.array(
                [agent_position[0], agent_position[2]], dtype=np.float32
            )
        else:
            return agent_position.astype(np.float32)
...
class GlobalCompassSensor(HeadingSensor):
    r"""The agents quaternions in world coordinates
    """

    def _get_uuid(self, *args: Any, **kwargs: Any):
        return "global_compass"

    def _get_observation_space(self, *args: Any, **kwargs: Any):
        return spaces.Box(low=-1., high=1., shape=(4,), dtype=np.float) # 4: quaternion size

    def get_observation(
        self, *args: Any, observations, episode, **kwargs: Any
    ):
        agent_state = self._sim.get_agent_state()
        rotation_world_agent = agent_state.rotation # x y z w
        rotation_world_agent = quaternion_from_coeff(quaternion.as_float_array(rotation_world_agent))
        return np.array(quaternion.as_float_array(rotation_world_agent), dtype=np.float32)
